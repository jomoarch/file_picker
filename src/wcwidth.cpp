...
static const interval wide[] = {
    ...
    {0x3131, 0x318E},   {0x3190, 0x31B7},   {0x31C0, 0x31E3},
...
bool bisearch(uint32_t ucs, const interval *table, int max) {
  int min = 0;
  int mid;
  if (ucs < table[0].first || ucs > table[max - 1].last)
    return false;
  while (max >= min) {
    mid = (min + max) / 2;
...
int mk_wcwidth(uint32_t ucs) {
  ...
  if (bisearch(ucs, combining,
               static_cast<int>(sizeof(combining) / sizeof(combining[0]))))
    return 0;
  if (bisearch(ucs, wide, static_cast<int>(sizeof(wide) / sizeof(wide[0]))))
    return 2;
  return 1;
}
