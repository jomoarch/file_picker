...
class Renderer {
public:
  // 一栏条目区在终端上的 1-based 行列范围（鼠标点击/滚轮按栏路由用，
  // 与 render 布局一致）
  struct EntryRegion {
    int col1 = 0; // 起始列
    int col2 = 0; // 结束列
  ...
  // 三栏条目区范围：left=父目录栏, middle=当前目录栏, right=光标子目录栏
  struct ColumnsRegion {
    EntryRegion left, middle, right;
  };
  ColumnsRegion columns_region(Size size) const;
